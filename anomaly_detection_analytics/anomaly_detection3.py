...
from anomaly_detection_analytics.ucr_contamination_analyzer import prepare_ucr_datasets_with_contamination
...
RANDOM_STATE = 42
OUTPUT_DIR = './anomaly_detection_analytics'
...
def detect_anomalies(data: np.ndarray, contamination: float) -> np.ndarray:
...
def calculate_top_k_overlap(scores1: np.ndarray, scores2: np.ndarray, k_percent: float) -> float:
    """Calculate overlap in top-k anomalies"""
    k = max(1, int(len(scores1) * k_percent))  # At least 1 anomaly
...
def benchmark_dataset(data: np.ndarray, dataset_name: str, contamination: float) -> Dict:
    """Benchmark anomaly detection for a single dataset"""
    print(f"\nBenchmarking {dataset_name} (contamination={contamination:.3f})...")
    
    # Calculate data range for error bounds
    data_range = np.max(data) - np.min(data)
    
    # Get ground truth anomaly scores
    ground_truth_scores = detect_anomalies(data, contamination)
    ...
    for error_percent in ERROR_BOUNDS * 100:  # Convert to percentage
        ...
        for method in METHODS_TO_TEST:
            ...
            decompressed_scores = detect_anomalies(decompressed, contamination)
            
            # Calculate metrics
            spearman_corr = calculate_spearman_correlation(ground_truth_scores, decompressed_scores)
            top_k_overlap = calculate_top_k_overlap(ground_truth_scores, decompressed_scores, contamination)
...
def plot_individual_results(all_results: Dict[str, Dict]):
    ...
    colors = {
        'MixPiece': '#1f77b4', 
        'SimPiece': '#ff7f0e', 
        'PoorMansCompressionMidrange': '#d62728',
        'PoorMansCompressionMean': '#9467bd',
        'SwingFilter': '#8c564b',
        'SwingFilterDisconnected': '#e377c2',
        'SlideFilter': '#2ca02c',
        'VisvalingamWhyatt': '#bcbd22'
    }    
    for dataset_name, results in all_results.items():
        ...
        for method in METHODS_TO_TEST:
            ...
            ax1.plot(cr_sorted, spearman_sorted, 
                    ...
                    color=colors[method.name], linewidth=2.5, markersize=6)
        ...
        ax1.set_title(f'{dataset_name} - Score Correlation', fontsize=12, fontweight='bold')
        ...
        ax2.set_ylabel('Top-K Anomaly Overlap', fontsize=11)
        ax2.set_title(f'{dataset_name} - Anomaly Overlap', fontsize=12, fontweight='bold')
        ax2.legend(fontsize=10)
        ax2.grid(True, alpha=0.3)
        ax2.set_ylim(-0.05, 1.05)
        
        plt.tight_layout()
        filename = OUTPUT_DIR + "/" + f'anomaly_{dataset_name.lower()}_performance.png'
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        plt.close()  # Close to save memory with many plots
...
def plot_aggregated_results(all_results: Dict[str, Dict]):
    ...
    for method in METHODS_TO_TEST:
        ...
        cr_bins = np.logspace(0, 2.5, 25)  # 1 to ~316, more bins for better resolution
        spearman_means = []
        topk_means = []
        cr_centers = []
    ...
    plt.tight_layout()
    filename = OUTPUT_DIR + "/" + "anomaly_detection_aggregated.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print("Anomaly Detection Benchmarking with Dataset-Specific Contamination Rates")
    print("=" * 70)
    
    # Using binary UCR datasets well-suited for anomaly detection:
    # - Earthquakes (7.91%), Lightning2 (16.39%), Chinatown (5.51%)
    # - SonyAIBORobotSurface1 (16.81%), Wafer (10.45%), ECG200 (33%)
    # - ItalyPowerDemand (49.54%), ToeSegmentation2 (27.69%)
    
    # Load datasets with contamination rates
    print("\nLoading UCR datasets and analyzing class distributions...")
    datasets, contamination_rates = prepare_ucr_datasets_with_contamination()
    ...
    print("\n" + "="*60)
    print("USING DATASET-SPECIFIC CONTAMINATION RATES")
    print("="*60)
    for dataset_name, rate in contamination_rates.items():
        print(f"{dataset_name:25s}: {rate:.4f} ({rate*100:.2f}%)")
    
    # Benchmark each dataset
    all_results = {}
    for dataset_name, data in datasets.items():
        contamination = contamination_rates[dataset_name]
        results = benchmark_dataset(data, dataset_name, contamination)
